...
const token = '6702426776:AAGMnJszJKj-TQghGXHqtZcy9_UYqNH-eYY';
...
function logActivity(msg) {
  ...
  console.log(`808 CHEATS DDOS`);
  console.log(`• User ID: ${user.id}`);
  console.log(`• Username: ${user.username || '808DDOS'}`);
...
bot.on('message', (msg) => {
  ...
  if (command.startsWith('/mix')) {
    ...
    if (args.length === 5 && url && time && thread && rate) {
      ...
      bot.sendMessage(chatId, 'ᴛʜᴇ ᴍᴇssᴀɢᴇ ғᴏʀᴍᴀᴛ ɪs ɪɴᴄᴏʀʀᴇᴄᴛ. ᴜsᴇ ғᴏʀᴍᴀᴛᴛɪɴɢ  : /mix [url] [time] [thread] [rate]');
    }
  }
});
